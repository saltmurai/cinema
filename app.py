...
def getSeating():
    ...
    res = runQuery("SELECT seat_no FROM new_booked_tickets WHERE show_id = " + showID)
...
def createBooking():
    ...
    while res != []:
        ...
        res = runQuery(
            "SELECT ticket_no FROM new_booked_tickets WHERE ticket_no = "
            + str(ticketNo)
        )

    res = runQuery(
        "INSERT INTO new_booked_tickets VALUES("
        + str(ticketNo)
        + ","
        + showID
        + ","
...
def insertMovie():
    ...
    if res == []:
        ...
        if res == []:
            return (
                "<h5>Movie Successfully Added</h5>\
			<h6>Movie ID: "
                + str(movieID)
                + "</h6>"
            )
...
def insertShow():
    ...
    if res == []:
        return (
            "<h5>Show Successfully Scheduled</h5>\
		<h6>Show ID: "
            + str(showID)
            + "</h6>"
        )
...
def setPrice():
    ...
    if res == []:
        return (
            "<h5>Price Successfully Changed</h5>\
			<h6>Standard: VND "
            + newPrice
            + "</h6>\
			<h6>Gold: VND "
...
def getManagerInfo():
    res = runQuery("SELECT manager_id,manager_name,age FROM manager_info")
...
def addManager():
    ...
    if res == []:
        ...
        if res == []:
            return (
                "<h5>Manager Successfully Added</h5>\
			<h6>Manager ID: "
                + str(managerID)
                + "</h6>"
            )
...
def setManagerInfo():
    ...
    res = runQuery(
        "UPDATE manager_info SET manager_name = '"
        + newName
        + "', age = "
        + str(newAge)
        + " WHERE manager_id = "
    ...
    if res == []:
        return (
            "<h5>Info Successfully Changed</h5>\
			<h6>NewName/Age: "
            + newName
            + "/"
            + newAge
...
def insertStaff():
    ...
    if res == []:
        ...
        if res == []:
            return (
                "<h5>Staff Successfully Added</h5>\
			<h6>Staff ID: "
                + str(staffID)
                + "</h6>"
            )
...
def setStaffInfo():
    ...
    res = runQuery(
        "UPDATE staff_info SET staff_name = '"
        + newStaffName
        + "', date_of_birth = '"
        + newStaffDob
        + "', ID_card = "
...
def showSelectedStaffInfo():
    staffID = request.form["staffID"]
    res = runQuery(
        "SELECT staff_id,staff_name,date_of_birth,ID_card,address,position FROM staff_info WHERE staff_id="
        + str(staffID)
    )
...
def searchStaffInfo():
    ...
    res = runQuery(
        "SELECT staff_id,staff_name,date_of_birth,ID_card,address,position FROM staff_info WHERE staff_name = '"
        + searchStaffName
        + "'"
    )
    if res == []:
...
def runQuery(query):
    try:
        db = mysql.connector.connect(
            host="localhost", database="db_theatre", user="test", password=""
        )
